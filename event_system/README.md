...
- **Foreign Key Constraints**: Cascading soft-deletes and strict relational integrity across `events`, `event_quotas`, `approval_bands`, `registrations`, and `registration_approvals`.
- **CSRF & XSS Protection**: All forms protected via Laravel `@csrf` and HTML-safe Blade component rendering.
